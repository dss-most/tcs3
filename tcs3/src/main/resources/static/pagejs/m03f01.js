...
var AppRouter = Backbone.Router.extend({
	initialize : function(options) {
		this.defaultBreadCrumb = Handlebars.compile($(
				"#defaultBreadCrumbTemplate").html());
		this.newBreadCrumb = Handlebars.compile($("#newBreadCrumbTemplate")
				.html());
		this.editBreadCrumb = Handlebars.compile($("#editBreadCrumbTemplate")
				.html());
		...
		this.tableResultView = new TableResultView({
			el : '#tableResultView'
		});
	...
	updateEditBreadCrump : function(reqNo) {
		this.$breadcrubmEl.html(this.editBreadCrumb({
			reqNo : reqNo
		}));
	},
...
var BarcodeModal = Backbone.View.extend({
	initialize : function(options) {
		this.barcodeFormTemplate = Handlebars.compile($("#barcodeFormTemplate")
				.html());
	},
	events : {
...
var OnlineBankModal = Backbone.View.extend({
	initialize : function(options) {
		this.barcodeFormTemplate = Handlebars.compile($(
				"#onlineBankFormTemplate").html());
	},
	events : {
...
var SearchView = Backbone.View.extend({
	...
	initialize : function(options) {
		this.searchViewTemplate = Handlebars.compile($("#searchViewTemplate")
				.html());
		this.orgSelectionTemplate = Handlebars.compile($(
				"#orgSelectionTemplate").html());
	...
	// Template
	// orgSelectionTemplate :
	// Handlebars.compile($("#orgSelectionTemplate").html()),
	...
	onChangeFormSlt : function(e) {
		...
		if (field == "sampleType") {
			model = App.Models.SampleType.findOrCreate({
				id : id
			});
			...
			if (id == 0) {
				...
				this.currentMainOrg = App.Models.Organization.findOrCreate({
					id : id
				});
				model = this.currentMainOrg;
				this.groupOrgCollection.url = appUrl('Organization') + '/'
						+ this.currentMainOrg.get('id') + '/children';
			...
			if (id == 0) {
				...
				this.currentGroupOrg = App.Models.Organization.findOrCreate({
					id : id
				});
	...
	render : function() {
		...
		json.sampleTypes.push({
			id : 0,
			nameTh : 'กรุณาเลือกประเภทตัวอย่าง'
		});
...
var TableResultView = Backbone.View
		.extend({
			initialize : function(options) {
				this.tableResultViewTemplate = Handlebars.compile($(
						'#tableResultViewTemplate').html());
			...
			onClickEditBtn : function(e) {
				e.preventDefault();
				var requestId = $(e.currentTarget).parents('tr')
						.attr('data-id');
				appRouter.navigate('Request/' + requestId, {
					trigger : true
				});
				return false;
			},
...
var NewRequestFromQuotationModal = Backbone.View
		.extend({
			initialize : function(options) {
				this.requstFromQuotationTemplate = Handlebars.compile($(
						'#requstFromQuotationTemplate').html());
				this.tableQuotationsResultViewTemplate = Handlebars.compile($(
						'#tableQuotationsResultViewTemplate').html());
				this.quotation = null;
			...
			onClickTemplateLnk : function(e) {
				var quotationId = $(e.currentTarget).parents('tr').attr(
						'data-id');

				this.quotation = App.Models.Quotation.findOrCreate({
					id : quotationId
				});
				this.quotation
						.fetch({
							...
							error : _
									.bind(
											function(model, response, options) {
												this
														.$(
																'#requestFromQuoationAlert')
														.html(
																'<div class="alert alert-warning" role="alert">ไม่พบหมายเลชใบเสนอราคาที่ระบุ กรุณาระบุใหม่</div>');
											}, this)
						});
			...
			renderQuotations : function() {
				...
				this.$el.find('#quotationsTable').html(
						this.tableQuotationsResultViewTemplate(json));

				return this;
			},
			render : function() {
				this.$el.find('.modal-body').html(
						this.requstFromQuotationTemplate());
...
var RequestAddressModalView = Backbone.View
		.extend({
			initialize : function(options) {
				if (options != null) {
				...
				this.requestAddressModalBodyTemplate = Handlebars.compile($(
						"#requestAddressModalBodyTemplate").html());
				this.districtSltTemplate = Handlebars.compile($(
						"#districtSltTemplate").html());
				// this.districtSltTemplate =
				// Handlebars.templates.DistrictSltTemplate;
			},
			events : {
													...
													function() {
														this
																.$(
																		'.modal-body')
																.prepend(
																		'<div class="alert alert-success" role="alert"><h4 style="margin-bottom:0px;"><i class="fa fa-check-square"></i> บันทึกข้อมูลเรียบร้อยแล้ว</h4></div>');
														// then update parent
														// view
														this.parentView.currentRequest
																.set(
																		this.titleField,
																		this.currentRequestAddress
																				.get('title'));
			...
			onChangeProvinceSlt : function(e) {
				...
				var province = App.Models.Province.findOrCreate({
					id : provinceId
				});
				this.currentRequestAddress
						.set('province', province.get('name'));
				this.currentRequestAddress.set('amphur', null);
			},
			onChangeDistrictSlt : function(e) {
				var districtId = $(e.currentTarget).val();
				var district = App.Models.District.findOrCreate({
					id : districtId
				});
			...
			renderDistrictSlt : function(provinceId, districtName) {
				this.districts
						.fetch({
							url : appUrl('Global/province/' + provinceId
									+ "/districts"),
							type : 'GET',
							success : _.bind(function() {
								var json = this.districts.toJSON();
								this.$el.find('#districtSltDiv').html(
										this.districtSltTemplate(json));

								if (districtName != null) {
									var district = this.districts.findWhere({
										name : districtName
									});

									this.$el.find('#districtSlt').val(
											district.get('id'));
								}
			...
			render : function() {
				...
				this.$el.find('.modal-body').html(
						this.requestAddressModalBodyTemplate(json));

				var currentProvinceId, currentDistrictId;

				var currentProvince = this.provinces.findWhere({
					name : this.currentRequestAddress.get('province')
				});
				...
				if (currentProvince != null) {
					...
					this.renderDistrictSlt(provinceId,
							this.currentRequestAddress.get('amphur'));
			...
			setCurrentRequestAddressAndRender : function(address, titleField,
					title) {
...
var CompanyModal = Backbone.View
		.extend({
			initialize : function(options) {
				this.chooseCompanyModalBodyTemplate = Handlebars.compile($(
						"#chooseCompanyModalBodyTemplate").html());
				this.companyViewTemplate = Handlebars.compile($(
						'#companyViewTemplate').html());
				this.personTblTemplate = Handlebars.compile($(
						'#personTblTemplate').html());
				this.addressTblTemplate = Handlebars.compile($(
						'#addressTblTemplate').html());
				this.personModalBodyTemplate = Handlebars.compile($(
						"#personModalBodyTemplate").html());
				this.companyViewButtonTemplate = Handlebars.compile($(
						"#companyViewButtonTemplate").html());
				this.chooseCompanyButtonTemplate = Handlebars.compile($(
						"#chooseCompanyButtonTemplate").html());
				this.personOrAddressButtonTemplate = Handlebars.compile($(
						"#personOrAddressButtonTemplate").html());
				this.companySearchTblTemplate = Handlebars.compile($(
						"#companySearchTblTemplate").html());
				...
				this.addressModalBodyTemplate = Handlebars.compile($(
						"#addressModalBodyTemplate").html());
				this.districtSltTemplate = Handlebars.compile($(
						"#districtSltTemplate").html());

				this.alertSuccessTempate = Handlebars.compile($(
						"#alertSuccessTempate").html());
			...
			search : function(pageNumber) {
				...
				this.companies.fetch({
					...
					success : _.bind(function(collection, response, options) {
						...
						this.$el.find('#companySearchTbl').html(
								this.companySearchTblTemplate(json));
					}, this)
				})
			},
			...
			onClickChooseBtn : function(e) {
				...
				var company = App.Models.Company.find({
					id : companyId
				});

				this.parentView.currentRequest.set('company', company);
				if (company.get('addresses').length == 0) {
					this.parentView.currentRequest.set('addressCompanyAddress',
							company.get('oldAddress'));
				} else {
					this.parentView.currentRequest.set('addressCompanyAddress',
							company.get('addresses').at(0));
				}

				this.parentView.currentRequest.set('contact', company.get(
						'people').at(0));
			...
			onClickSaveBtn : function(e) {
				...
				this.currentCompany.save(null, {
					success : _.bind(function(model, response, options) {
						...
						this.$el.find("#companyViewInfo").html(
								this.alertSuccessTempate({
									icon : "fa-check",
									message : "บันทึกข้อมูลลงฐานข้อมูลแล้ว"
								}));
						this.parentView.renderCompany();
					}, this)
				});
			...
			onClickRemoveAddressBtn : function(e) {
				...
				var r = confirm('คุณต้องการลบรายการที่อยู่ '
						+ item.get('line1') + " " + item.get('line2'));
				if (r == true) {
			...
			onClickSavePersonBtn : function(e) {
				...
				this.$el
						.find("#personModalInfo")
						.html(
								this
										.alertSuccessTempate({
											icon : "fa-check",
											message : "บันทึกข้อมูลแล้ว กรุณากลับไปหน้าเดิมและกดบันทึกอีกรอบข้อมูลจึงจะบันทึกลงในฐานข้อมูล"
										}));
			...
			onClickRemovePersonBtn : function(e) {
				...
				var r = confirm('คุณต้องการลบรายการผู้ติดต่อ '
			...
			// }
			//		 
			// this.parentView.currentRequest.set('contact',
			// company.get('people').at(0));
			//		 
			// this.parentView.renderCompany();
			//		 
			// this.$el.modal('hide');
			...
			renderPerson : function(customer) {
				...
				this.$el.find('.modal-header span').html(
						this.currentCompany.get('nameTh')
								+ ": แก้ไขรายการผู้ติดต่อ");
				var json = customer.toJSON();

				this.$el.find('.modal-body').html(
						this.personModalBodyTemplate(json));
				this.$el.find('.modal-footer').html(
						this.personOrAddressButtonTemplate({
							model : "Person"
						}));
			...
			renderAddress : function(address) {
				...
				this.$el.find('.modal-header span').html(
						this.currentCompany.get('nameTh') + ": แก้ไขที่อยู่");
				var json = address.toJSON();
				json.provinces = this.provinces.toJSON();

				this.$el.find('.modal-body').html(
						this.addressModalBodyTemplate(json));
				if (this.currentAddress.get('province') != null
						&& this.currentAddress.get('province').get('id') != null) {
					var provinceId = this.currentAddress.get('province').get(
							'id');
					this.$el.find('#provinceSlt').val(
							this.currentAddress.get('province').get('id'));
					var districtId = null;
					if (this.currentAddress.get('district') != null
							&& this.currentAddress.get('district').get('id') != null) {
						districtId = this.currentAddress.get('district').get(
								'id');
					}
			...
			renderPersonTbl : function() {
				...
				this.$el.find('#personTbl').html(this.personTblTemplate(json));
				return this;
			},
			...
			renderChooseCompany : function() {
				...
				this.$el.find('.modal-body').html(
						this.chooseCompanyModalBodyTemplate());
				this.$el.find('#companySrh').search();

				this.$el.find('.modal-footer').html(
						this.chooseCompanyButtonTemplate());
			...
			render : function() {
				this.$el
						...
						.html(
								'<i class="fa fa-pencil-square-o"></i> แก้ไขข้อมูลที่อยู่บริษัทลูกค้า');

				var json = this.currentCompany.toJSON();
				this.$el.find('.modal-body').html(
						this.companyViewTemplate(json));
				this.renderAddressTbl();
				this.renderPersonTbl();

				this.$el.find('.modal-footer').html(
						this.companyViewButtonTemplate());
...
var TestMethodItemModal = Backbone.View
		.extend({
			/**
			 * @memberOf TestMethodItemModal
			 */
			initialize : function(options) {
				this.testMethodGroupModalBodyTemplate = Handlebars.compile($(
						'#testMethodGroupModalBodyTemplate').html());
				this.testMethodItemModalBodyTemplate = Handlebars.compile($(
						'#testMethodItemModalBodyTemplate').html());
				this.testMethodSearchTblTemplate = Handlebars.compile($(
						'#testMethodSearchTblTemplate').html());
				this.mode = "";
			...
			search : function(pageNumber) {
				...
				this.testMethods.fetch({
					url : appUrl('TestMethod/findByNameOrCode/page/'
							+ pageNumber),
					type : 'POST',
					data : {
						query : query
					...
					success : _.bind(function(collection, response, options) {
						...
						if (this.mode == "newTestMethodItem"
								|| this.mode == "newTestMethodItemAllSample") {
							json.editMode = false;
							this.$el.find('#testMethodSearchTbl').html(
									this.testMethodSearchTblTemplate(json));
						} else {
							json.editMode = true;
							this.$el.find('#testMethodSearchTbl').html(
									this.testMethodSearchTblTemplate(json));
			...
			onClickTestMethodRdo : function(e) {
				...
				var testMethod = App.Models.TestMethod.find({
					id : testMethodId
				});
			...
			onClickSaveBtn : function(e) {
				if (this.mode == "editTestMethodItem") {
					var testMethodId = this.$el.find('.testMethodRdo:checked')
							.val();
							...
							.find(
									function(item) {
										if (item.get('testMethod') != null) {
											return item.get('testMethod').get(
													'id') == testMethod
													.get('id');
										}
										return false;
					...
					this.selected
							.forEach(
									function(testMethod, index, list) {
										...
										var findItem = this.currentSample
												.get('jobs')
												.find(
														function(item) {
															if (item
										...
										if (findItem == null) {
											this.currentSample.get('jobs').add(
													item);
										}
									}, this);
					...
					// do save
					this.currentRequest.get('testMethodItems').add(
							this.currentItem);
				}
			...
			render : function() {
				...
				if (this.mode == "newTestMethodItem"
					...
					this.$el.find('.modal-header span')
							.html("เพิ่มรายการทดสอบ");
					this.$el.find('.modal-body').html(
							this.testMethodItemModalBodyTemplate());
					this.$el.find('#testMethodSrh').search();

				} else if (this.mode == "newTestMethodGroup") {
					this.currentItem = new App.Models.TestMethodQuotationItem();
					this.$el.find('.modal-header span').html(
							"เพิ่มกลุ่มรายการทดสอบ");
					this.$el.find('.modal-body').html(
							this.testMethodGroupModalBodyTemplate());

				} else if (this.mode == "editTestMethodGroup") {
					this.$el.find('.modal-header span').html(
							"แก้ไขกลุ่มรายการทดสอบ");
					json = this.currentItem.toJSON();
					this.$el.find('.modal-body').html(
							this.testMethodGroupModalBodyTemplate(json));
				} else if (this.mode = "editTestMethodItem") {
					this.$el.find('.modal-header span')
							.html("แก้ไขรายการทดสอบ");
					json = this.currentItem.toJSON();
					this.$el.find('.modal-body').html(
							this.testMethodItemModalBodyTemplate(json));
					this.$el.find('#testMethodSrh').search();
				}
...
var FormView = Backbone.View
		.extend({
			/**
			 * @memberOf FormView
			 */
			initialize : function(options) {
				this.requestViewTemplate = Handlebars.compile($(
						"#requestViewTemplate").html());
				this.orgSelectionTemplate = Handlebars.compile($(
						"#orgSelectionTemplate").html());

				this.labJobTblTemplate = Handlebars.compile($(
						"#labJobTblTemplate").html());

				this.companyInfoTemplate = Handlebars.compile($(
						"#companyInfoTemplate").html());

				this.sampleViewTemplate = Handlebars.compile($(
						"#sampleViewTemplate").html());
				this.currentRequest = null;

				this.testMethodItemModal = new TestMethodItemModal({
					el : '#testMethodModal'
				});
				...
				this.requestAddressModal = new RequestAddressModalView({
					el : '#requestAddressModal'
				});
			...
			onClickEditRequestAddressBtn : function(e) {
				...
				var titleField = $('span[data-id="' + reqAddrId + '"]').attr(
						'data-titleField');

				var currentAddr = App.Models.LabAddress.findOrCreate(reqAddrId);

				this.requestAddressModal.setCurrentRequestAddressAndRender(
						currentAddr, titleField, title);
			...
			onAddressSltLstChange : function(e, data) {
				...
				target = App.Models.Address.find({
					id : data.value
				});
			...
			onSltChange : function(e) {
				...
				if (field == "mainOrg") {
					target = App.Models.Organization.find({
						id : value
					});
					...
					target = App.Models.SampleType.find({
						id : value
					});
				} else if (field == "contact") {
					target = App.Models.Customer.find({
						id : value
					});
				...
				} else if ((field == "invoiceAddressCompanyAddress" || field == "reportAddressCompanyAddress")
						&& value != 0) {
					target = App.Models.Address.find({
						id : value
					});
			...
			onRdoChange : function(e) {
				...
				if (field == 'sampleOrg' && value == 'SELF') {
					...
					var html = sltInputHtml(mainOrgs.toJSON(), "id", "name",
							null, "sampleReceiverOrg", "กรุณาเลือกหน่วยงาน",
							"หน่วยงานที่รับตัวอย่าง", 3, 9, "required");

					el.html(html.string);

				} else if (field == 'sampleOrg' && value == 'SARABUN') {
					var el = $('#sampleReceiverOrgDiv');
					var html = sltInputHtml(sarabunOrgs.toJSON(), "id", "name",
							null, "sampleReceiverOrg", "",
							"หน่วยงานที่รับตัวอย่าง", 3, 9, "required");
					this.currentRequest.set('sampleReceiverOrg', sarabunOrgs
							.at(0));
					el.html(html.string);
				}
			...
			onClickChooseCompanyBtn : function() {
				if (this.currentQuotation != null) {
					this.companyModal.currentCompany = this.currentQuotation
							.get('company');
					;
				} else {
			...
			onClickPromotionCbx : function(e) {
				...
				var promotion = App.Models.Promotion.findOrCreate({
					id : promotionId
				});
				...
				if (promotionCheck) {
					...
					var pdFound = promotions.find(function(pd) {
						return pd.get('promotion').get('id') == promotionId
					});
			...
			calculateTotal : function() {
				...
				this.currentRequest.get('samples').each(
						function(sample, sampleIndex) {
							var sumSample = 0;

							sample.get('jobs').each(
									function(job) {
										if (job.get('quantity') != null
												&& job.get('quantity') > 0) {
											sumTotal += job.get('quantity')
													* job.get('testMethod')
															.get('fee');
											sumSample += job.get('quantity')
													* job.get('testMethod')
															.get('fee');
										}
									});
							...
							$("#labJobTbl_" + sampleIndex + " .sumTotalItem")
									.html(__addCommas(sumSample));
				...
				this.$el.find('#sumTotalItem').html(
						"<b>" + __addCommas(sumTotal) + "</b>");
				this.$el.find('#sampleNumSpan').html(
						__addCommas(this.currentRequest.get('samples').length));
				...
				if (this.currentRequest.get('promotions') != null
					...
					for (var i = 0; i < promotions.length; i++) {
						...
						var discount = (sumTotal * pd.get('promotion').get(
								'percentDiscount')) / 100;
						...
						this.$el.find(
								'#promotion_' + pd.get('promotion').get('id'))
								.html("<b>" + __addCommas(discount) + "</b>");
					}
				}
				...
				if (invoice != null) {
					sumTotal = sumTotal + (invoice.get('copyFee')==null?0:invoice.get('copyFee'));
					sumTotal = sumTotal + (invoice.get('translateFee')==null?0:invoice.get('translateFee'));
					sumTotal = sumTotal + (invoice.get('coaFee')==null?0:invoice.get('coaFee'));
					sumTotal = sumTotal + (invoice.get('etcFee')==null?0:invoice.get('etcFee'));
				...
				this.$el.find('#sumTotal').html(
						"<b>" + __addCommas(sumTotal) + "</b>");

			},
			onChangeSampleNumSbx : function(e, v) {
				var sampleIndex = $(e.currentTarget).parents('div.panel').attr(
						'data-sampleIndex');
			...
			onChangeItemQuantitySbx : function(e, v) {
				// see where is click
				var index = $(e.currentTarget).parents('tr').attr('data-index');
				var sampleIndex = $(e.currentTarget).parents('div.samplePanel')
						.attr('data-sampleIndex');

				var invoice = this.currentRequest.get('invoices').at(0);

				if (!isNaN(index)) {
					var item = this.currentRequest.get('samples').at(
							sampleIndex).get('jobs').at(index);
					item.set('quantity', v);

					$(e.currentTarget).parent().next().html(
							__addCommas(item.get('quantity')
									* item.get('testMethod').get('fee')));
				} else {
					var field = $(e.currentTarget).find('input').attr(
							'data-field');
					...
					if (field == 'etcFee') {
						...
						invoice.set(field, v);
						var forField = $(e.currentTarget).attr(
								'data-calculateForField');
			...
			onClickRemoveItem : function(e) {
				...
				var sampleIndex = $(e.currentTarget).parents('div.samplePanel')
						.attr('data-sampleIndex');
				var itemIndex = $(e.currentTarget).parents('tr').attr(
						'data-index');
				...
				if (item.get('testMethod') != null) {
					str = 'คุณต้องการลบรายการทดสอบ '
							+ item.get('testMethod').get('code') + ' ?';
				} else {
			...
			onClickNewLabJobBtn : function(e) {
				var sampleIndex = $(e.currentTarget).parents('div.samplePanel')
						.attr('data-sampleIndex');
				var targetSample = this.currentRequest.get('samples').at(
						sampleIndex);
			...
			onClickCollapseAllSampleBtn : function(e) {
				...
				$('.samplePanelCollapse').find('i').removeClass(
						'fa-chevron-down');
				$('.samplePanelCollapse').find('i')
			...
			onClickCopySampleBtn : function(e) {
				var sampleIndex = $(e.currentTarget).parents('div.samplePanel')
						.attr('data-sampleIndex');
				var sourceSample = this.currentRequest.get('samples').at(
						sampleIndex);
			...
			onClickNewSampleBtn : function(e) {
				...
				var newDiv = $('div.samplePanel[data-sampleindex='
						+ newSampleIndex + ']');
				setTimeout(function() {
			...
			onClickItem : function(e) {
				...
				var sampleIndex = $(e.currentTarget).parents('div.samplePanel')
						.attr('data-sampleIndex');
				var currentSample = this.currentRequest.get('samples').at(
						sampleIndex);
			...
			onMainGroupChange : function(e) {
				...
				if (mainGroupId == 0) {
					...
					var groupOrg = App.Models.Organization.findOrCreate({
						id : mainGroupId
					});
			...
			onSaveBtn : function(e) {
				...
				this.currentRequest.save(null, {
					success : _.bind(function(model, response, options) {
						...
						this.currentRequest.set('companyName',
								response.data.companyName);

						this.currentRequest.set('address',
								response.data.address);
						this.currentRequest.set('invoiceAddress',
								response.data.invoiceAddress);
						this.currentRequest.set('reportAddress',
								response.data.reportAddress);

						var samples = response.data.samples;

						for (var i = 0; i < samples.length; i++) {
							var thisSample = this.currentRequest.get('samples')
									.at(i);
						...
						appRouter.navigate("Request/"
								+ this.currentRequest.get('id'), {
							trigger : true,
							replace : true
						});

						appRouter.updateEditBreadCrump(this.currentRequest
								.get("reqNo"));
			...
			newRequest : function(quotationId) {
				...
				var quotation;
				if (quotationId != null) {
					quotation = App.Models.Quotation.findOrCreate({
						id : quotationId
					});
					quotation.fetch({
						success : _.bind(function() {
							...
							request.set('company', this.currentQuotation
									.get('company'));
							request.set('addressCompanyAddress',
									this.currentQuotation.get('address'));
							request.set('invoiceAddressCompanyAddress',
									this.currentQuotation.get('address'));
							request.set('reportAddressCompanyAddress',
									this.currentQuotation.get('address'));
							request.set('addressTitle', this.currentQuotation
									.get('company').get('nameTh'));
							request.set('invoiceTitle', this.currentQuotation
									.get('company').get('nameTh'));
							request.set('reportTitle', this.currentQuotation
									.get('company').get('nameTh'));

							request.set('contact', this.currentQuotation
									.get('contact'));
							request.set('mainOrg', this.currentQuotation
									.get('mainOrg'));
							request.set('sampleType', this.currentQuotation
									.get('sampleType'));
							request.set('estimatedWorkingDay',
									this.currentQuotation.get('estimatedDay'));

							for (var i = 0; i < this.currentQuotation
									.get('sampleNum'); i++) {
								var sample = new App.Models.RequestSample();
								sample.set('item', 1);
								sample.set('request', request);
								for (var j = 0; j < this.currentQuotation
										.get('testMethodItems').length; j++) {
									var item = this.currentQuotation.get(
											'testMethodItems').at(j);

									if (item.get('testMethod') != null) {
										var job = new App.Models.LabJob();
										job.set('testMethod', item
												.get('testMethod'));
										job.set('fee', item.get('fee'));
										job.set('quantity', item
												.get('quantity'));
			...
			editRequest : function(id) {
				...
				this.currentRequest = App.Models.Request.findOrCreate({
					id : id
				});
				this.currentRequest.fetch({
					success : _.bind(function() {
						if (this.currentRequest.get('sampleReceiverOrg').get(
								'id') == 3) {
							this.currentRequest.get('sampleReceiverOrg').set(
									'name', 'ฝ่ายสารบรรณ');
			...
			reorderItem : function(e, ui) {
				...
				var sampleIndex = $(e.target).parent('table').attr(
						'data-sampleIndex');
				...
				$("#labJobTbl_" + sampleIndex + " tbody tr").each(
						function(index, tr) {
			...
			renderLabJobTbl : function(targetSample) {
				var sampleIndex = this.currentRequest.get('samples').indexOf(
						targetSample);
					...
					if (json.jobs != null) {
						...
						for (var i = 0; i < json.jobs.length; i++) {
							if (json.jobs[i].testMethod != null) {
								...
								total += (json.jobs[i].quantity)
										* (json.jobs[i].fee);
								json.jobs[i].totalLine = (json.jobs[i].quantity)
										* (json.jobs[i].fee);
							}
						}
						json.totalItems = total;

						json.totalItemSampleNum = json.totalItems
								* json.sampleNum;

						this.$el.find(
								"div.samplePanel[data-sampleIndex="
										+ sampleIndex + "] table.labJobTbl")
								.html(this.labJobTblTemplate(json));
			...
			renderCompany : function() {
				...
				// if(json.address.line1 == null || json.address.line1.length ==
				// 0) {
				...
				if (this.currentRequest.get('company') != null) {
					...
					if (this.currentRequest.get("reqNo") != null) {
						...
						json.company.addressDisplay = "<span data-titleField='addressTitle' data-id='"
								+ this.currentRequest.get('address').get('id')
								+ "'>"
								+ this.currentRequest.get("addressTitle")
								+ "</span><br/>";
						json.company.addressDisplay += this.currentRequest.get(
								'address').get('address')
								+ " "
								+ this.currentRequest.get('address').get(
										'amphur')
								+ " "
								+ this.currentRequest.get('address').get(
										'province')
						...
						json.company.reportAddressDisplay = "<span data-titleField='reportTitle' data-id='"
								+ this.currentRequest.get('reportAddress').get(
										'id')
								+ "'>"
								+ this.currentRequest.get("reportTitle")
								+ "</span><br/>";
						json.company.reportAddressDisplay += this.currentRequest
								.get('reportAddress').get('address')
								+ " "
								+ this.currentRequest.get('reportAddress').get(
										'amphur')
								+ " "
						...
						json.company.receiptAddressDisplay = "<span data-titleField='invoiceTitle' data-id='"
								+ this.currentRequest.get('invoiceAddress')
										.get('id')
								+ "'>"
								+ this.currentRequest.get("invoiceTitle")
								+ "</span><br/>";
						json.company.receiptAddressDisplay += this.currentRequest
								.get('invoiceAddress').get('address')
								+ " "
								+ this.currentRequest.get('invoiceAddress')
										.get('amphur')
								+ " "
					...
					this.$el.find("#companyNameThTxt")
							.html(json.company.nameTh);
					this.$el
							.find("#editCompanyBtn")
							.html(
									'<button id="companyBtn" type="button" class="btn btn-primary"><i class="fa fa-address-card-o"></i> แก้ไขข้อมูลบริษัทลูกค้า </button>');
					...
					if (this.currentQuotation != null) {
						...
						address = this.currentRequest.get('company').get(
								'addresses');
						contact = this.currentRequest.get('customer');
					}

					if (contact == null) {
						json.company.people.unshift({
							id : 0,
							firstName : 'กรุณาเลือกผู้ติดต่อ'
						});
					} else {
					...
					json.company.receiptAddresses = this.currentRequest.get(
							'company').get('addresses').toJSON();
					json.company.reportAddresses = this.currentRequest.get(
							'company').get('addresses').toJSON();

					if (address == null) {
						json.company.addresses.unshift({
							id : 0,
							line1 : 'กรุณาเลือกที่อยู่'
						});
					} else {
					...
					if (this.currentRequest.get('receiptAddresses') == null) {
						json.company.receiptAddresses.unshift({
							id : 0,
							line1 : 'เหมือนที่อยู่ด้านบน',
							selected : 'selected'
						});
					} else {
						__setSelect(json.company.receiptAddresses,
								this.currentRequest.get('receiptAddresses'));
					}

					if (this.currentRequest.get('reportAddresses') == null) {
						json.company.reportAddresses.unshift({
							id : 0,
							line1 : 'เหมือนที่อยู่ด้านบน',
							selected : 'selected'
						});
					} else {
						__setSelect(json.company.reportAddresses,
								this.currentRequest.get('reportAddresses'));
					}

					this.$el.find('#companyInfoDiv').html(
							this.companyInfoTemplate(json));
					...
					if (json.hasRequestNo) {
						...
						this.$el
								.find('label[for="addressDisplayTxt"]')
								.append(
										'<button id="editAddressBtn" data-id="'
												+ this.currentRequest.get(
														'address').get('id')
												+ '" type="button" style="margin-left:14px;" class="btn btn-primary btn-xs editRequestAddressBtn"><i class="fa fa-edit"></i> แก้ไข</button>');
						this.$el
								.find('label[for="receiptAddressDisplayTxt"]')
								.append(
										'<button id="editReceiptAddressBtn" data-id="'
												+ this.currentRequest.get(
														'invoiceAddress').get(
														'id')
												+ '" type="button" style="margin-left:14px;" class="btn btn-primary btn-xs editRequestAddressBtn"><i class="fa fa-edit"></i> แก้ไข</button>');
						this.$el
								.find('label[for="reportAddressDisplayTxt"]')
								.append(
										'<button id="editReportAddressBtn" data-id="'
												+ this.currentRequest.get(
														'reportAddress').get(
														'id')
												+ '" type="button" style="margin-left:14px;" class="btn btn-primary btn-xs editRequestAddressBtn"><i class="fa fa-edit"></i> แก้ไข</button>');
			...
			render : function() {
				...
				if (promotions.length > 0) {
					...
					if (this.currentRequest.get('promotions') != null
							&& this.currentRequest.get('promotions').length > 0) {
						// we have promotion
						for (var i = 0; i < this.currentRequest
								.get('promotions').length; i++) {
							var promotion_id = this.currentRequest.get(
									'promotions').at(i).get('promotion').get(
									'id');
				...
				if (this.currentRequest.get('sampleOrgSELF') == true) {
					var el = $('#sampleReceiverOrgDiv');
					var html = sltInputHtml(mainOrgs.toJSON(), "id", "name",
							this.currentRequest.toJSON(), "sampleReceiverOrg",
							"กรุณาเลือกหน่วยงาน", "หน่วยงานที่รับตัวอย่าง", 3,
							9, "required");
